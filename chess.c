...
void intro() {
    ...
    for(int i = 0; i<73; i++)
    	printf(" ");
    printf("\033[38;2;186;21;21m Press any key to start, press q to quit: \033[0m");
    char input;
    input = fgetc(stdin);
        ...
        for(int j = 0; j<9; j++) {
            ...
            if(j == 8) {
                ...
                if(board[i][j]) {
                    if((i+j)%2==0) {
                    	if(moves[i][j] == 3)
                    		if((board[i][j] == 'K') && checked)
                    			printf("\033[48;2;211;215;207;38;2;255;0;0m %s \033[0m", unicode_symbols(board[i][j]));
                    		else
                    			printf("\033[48;2;211;215;207;38;2;0;153;153m %s \033[0m", unicode_symbols(board[i][j]));
                    	else if(moves[i][j] == 2)
                    		printf("\033[48;2;211;215;207;38;2;255;0;0m %s \033[0m", unicode_symbols(board[i][j]));
                    	else
                    		if((board[i][j] == 'K') && checked)
                    	...
                    	if(moves[i][j] == 3)
                    		if((board[i][j] == 'K') && checked)
                    			printf("\033[48;2;139;69;19;38;2;255;0;0m %s \033[0m", unicode_symbols(board[i][j]));
                    		else
                    			printf("\033[48;2;139;69;19;38;2;0;153;153m %s \033[0m", unicode_symbols(board[i][j]));
                    	else if(moves[i][j] == 2)
                    		printf("\033[48;2;211;215;207;38;2;255;0;0m %s \033[0m", unicode_symbols(board[i][j]));
                    	else
                    		if((board[i][j] == 'K') && checked)
                        ...
                        if ((i+j)%2==0) {
                        	if(moves[i][j] == 1)
                    			printf("\033[48;2;211;215;207;38;2;102;0;102m · \033[0m");
                    		else
                    			printf("\033[48;2;211;215;207;38;2;0;0;0m   \033[0m");
                    	} else {
                    		if(moves[i][j] == 1)
                    			printf("\033[48;2;139;69;19;38;2;102;0;102m · \033[0m");
                    		else
...
//Check what piece is attacking the player king
bool check_for_attack(char piece) {
	for(int i = 0; i<8; i++) {
		for(int j = 0; j<8; j++) {
...
int main() {
    ...
    while(true){
        ...
        system("clear");
        if(checked)
        	printf("\033[38;2;255;0;0m YOUR KING IS BEING ATTACKED! \033[0m\n\n");
        if(current_player == 'W')
        	printf("White's turn\n");
        else
        	printf("Black's turn\n");
